...
const contextFor = async (
  ...
  const { data, error } = await database.rpc('planaria_get_mod_media_upload_context', {
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (input.action === 'create') {
      if (!safeIdentifier(input.modId) || !['image/png', 'image/jpeg', 'image/webp'].includes(String(input.mimeType)) ||
          !Number.isSafeInteger(input.expectedSize) || Number(input.expectedSize) <= 0 ||
          Number(input.expectedSize) > 20971520 || !sha256String(input.expectedSha256)) throw new Error('invalid-request');
      const storage = await signerRequest('/media/multipart/create', {
        purpose: 'mod-image', modId: input.modId, mimeType: input.mimeType,
      ...
      const { data, error } = await context.database.rpc('planaria_create_mod_media_upload', {
        subject: context.subject, target_mod: input.modId, storage_provider: 's3-compatible',
    ...
    if (input.action === 'sign-part') {
      ...
      const storage = await signerRequest('/multipart/sign-part', {
        purpose: 'mod-image', objectKey: stored.object_key,
    ...
    if (input.action === 'finalize') {
      ...
      const storage = await signerRequest('/media/multipart/finalize', {
        purpose: 'mod-image', objectKey: stored.object_key,
        uploadId: stored.provider_upload_id, parts: input.parts,
      });
      if (!Number.isSafeInteger(storage.size) || !sha256String(storage.sha256) ||
          !['image/png', 'image/jpeg', 'image/webp'].includes(String(storage.mimeType)) ||
          !Number.isInteger(storage.width) || !Number.isInteger(storage.height)) throw new Error('storage-provider-failed');
      const { data, error } = await context.database.rpc('planaria_finalize_mod_media_upload', {
      ...
      if (error || !uuidString(data)) throw new Error('request-failed');
      return json(200, { mediaId: data, verified: true });
    }
    if (input.action === 'abort') {
      ...
      await signerRequest('/multipart/abort', {
        purpose: 'mod-image', objectKey: stored.object_key, uploadId: stored.provider_upload_id,
      });
      const { error } = await context.database.rpc('planaria_abort_mod_media_upload', {
